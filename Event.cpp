...
void Event::initDB() {
	...
	*tutorial = {
	   ...
	   { "당신은 신입 모험가입니다.", "이름과 스테이터스를 설정하고 ","모험을 떠나세요." },
		{ { "내가 누구냐면...", { 0, 0, 0, 0, EventAction::None }}},
	...
	*story2 = {
		...
		{ { "물을 건넨다", { 5, -3, 20, 0, EventAction::None } },
		  { "거절하고 지나간다", { 0, 0, 0, 0, EventAction::None } } },
	...
	*story3 = {
		...
		{ { "검을 뽑는다", { -10, 0, 30, 0, EventAction::Battle, -1, { static_cast<int>(MonsterId::CursedArmor) } } },
		  { "조용히 기도한다", { 10, 5, 10, 0, EventAction::None } } },
	...
	*battle1 = {
		...
		{ { "전투를 시작한다", { 0, 0, 0, 0, EventAction::Battle, -1, { static_cast<int>(MonsterId::HungryWolf) } } },
		  { "금화를 던져 유인한다", { 0, 0, 0, -10, EventAction::None } } },
	...
	*battle2 = {
		...
		{ { "정면으로 맞선다", { 0, 0, 0, 0, EventAction::Battle, -1, { static_cast<int>(MonsterId::Goblin), static_cast<int>(MonsterId::Goblin) } } },
		  { "빠르게 도망친다", { -8, -5, 0, 0, EventAction::None } } },
	...
	*battle3 = {
		...
		{ { "성스러운 문양을 확인한다", { -3, -5, 15, 0, EventAction::Battle, -1, { static_cast<int>(MonsterId::CursedArmor) } } },
		  { "전투 준비를 한다", { 0, 0, 0, 0, EventAction::Battle, -1, { static_cast<int>(MonsterId::CursedArmor) } } } },
	...
	*shop1 = {
		...
		{ { "회복약을 산다", { 20, 10, 0, -15, EventAction::Shop } },
		  { "물건을 구경한다", { 0, 0, 0, 0, EventAction::Shop } } },
	...
	*shop3 = {
		...
		{ { "지도 조각을 산다", { 0, 0, 25, -30, EventAction::Shop } },
		  { "다음에 사기로 한다", { 0, 0, 0, 0, EventAction::None } } },
	...
	*rest1 = {
		...
		{ { "잠시 쉰다", { 25, 15, 0, 0, EventAction::Rest } },
		  { "불을 끄고 떠난다", { 0, 0, 0, 0, EventAction::None } } },
	...
	*rest2 = {
		...
		{ { "물을 마신다", { 15, 20, 0, 0, EventAction::Rest } },
		  { "물통만 채운다", { 5, 5, 0, 0, EventAction::None } } },
	...
	*treasure2 = {
		...
		{ { "짐칸을 뒤진다", { -5, 0, 15, 50, EventAction::None } },
		  { "바퀴 아래를 살핀다", { 0, 0, 25, 20, EventAction::None } } },
	...
	*treasure3 = {
		...
		{ { "광석을 캔다", { -10, -10, 25, 60, EventAction::None } },
		  { "작은 조각만 챙긴다", { 0, -3, 10, 20, EventAction::None } } },
	...
	*boss1 = {
		...
		{ { "수호자에게 도전한다", { 0, 0, 0, 0, EventAction::Battle, -1, { static_cast<int>(MonsterId::ForestGuardian) } } },
		  { "숲에 사과한다", { -5, -10, 10, 0, EventAction::Battle, -1, { static_cast<int>(MonsterId::ForestGuardian) } } } },
	...
	*boss2 = {
		...
		{ { "정정당당히 결투한다", { 0, 0, 0, 0, EventAction::Battle, -1, { static_cast<int>(MonsterId::RedMoonKnight) } } },
		  { "선제 공격을 시도한다", { -10, -5, 10, 0, EventAction::Battle, -1, { static_cast<int>(MonsterId::RedMoonKnight) } } } },
...
std::shared_ptr<EventData> Event::getRandomEventData(int tmpId)
{
	...
	for (const auto& entry : eventDB) {
		...
		if (eventData && eventData->type != EventType::Empty && entry.first != tmpId) {
...
std::shared_ptr<EventData> Event::getRandomEventData(int tmpId,EventType type)
{
	...
	for (const auto& entry : eventDB) {
		const auto& eventData = entry.second;
		if (eventData && eventData->type == type && entry.first != tmpId) {
			candidates.push_back(eventData);
		}
	}
