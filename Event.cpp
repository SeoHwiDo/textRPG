...
void Event::initDB() {
	...
	*story1 = {
		...
		{ { "연기를 따라간다", { -5, 0, 15, 0, EventAction::Battle } },
	...
	*story3 = {
		...
		{ { "검을 뽑는다", { -10, 0, 30, 0, EventAction::Battle } },
	...
	*battle1 = {
		...
		{ { "전투를 시작한다", { 0, 0, 0, 0, EventAction::Battle } },
	...
	*battle2 = {
		...
		{ { "정면으로 맞선다", { 0, 0, 0, 0, EventAction::Battle } },
	...
	*battle3 = {
		...
		{ { "성스러운 문양을 확인한다", { -3, -5, 15, 0, EventAction::Battle } },
		  { "전투 준비를 한다", { 0, 0, 0, 0, EventAction::Battle } } }
	};
	...
	*boss1 = {
		...
		{ { "수호자에게 도전한다", { 0, 0, 0, 0, EventAction::Battle } },
		  { "숲에 사과한다", { -5, -10, 10, 0, EventAction::Battle } } }
	};
	...
	*boss2 = {
		...
		{ { "정정당당히 결투한다", { 0, 0, 0, 0, EventAction::Battle } },
		  { "선제 공격을 시도한다", { -10, -5, 10, 0, EventAction::Battle } } }
	};
	...
	*boss3 = {
		...
		{ { "최후의 전투를 시작한다", { 0, 0, 0, 0, EventAction::Battle } },
		  { "마력을 모두 끌어낸다", { -15, -30, 20, 0, EventAction::Battle } } }
	};
...
//전체 랜덤 호출
std::shared_ptr<EventData> Event::getRandomEventData()
{
	...
	for (const auto& entry : eventDB) {
		...
		if (eventData && eventData->type != EventType::Empty) {
...
std::shared_ptr<EventData> Event::getRandomEventData(EventType type)
{
	std::vector<std::shared_ptr<EventData>> candidates;
	for (const auto& entry : eventDB) {
		const auto& eventData = entry.second;
...
std::shared_ptr<EventData> Event::getRandomEventData(std::initializer_list<EventType> types)
{
	std::vector<std::shared_ptr<EventData>> candidates;
	for (auto type : types) {
		for (const auto& entry : eventDB) {
